...
def get_batch_master(arg_param):
    ...
    try:
        if arg_param == 'full':
            ...
            # 지적도 올리기
            strSQL = "SELECT DATA_NM, LAYER_YN, LAST_DAILY_UPDATE, LAST_FULL_UPDATE FROM T_LNDB_L_BATCH_MNG WHERE USE_YN = 'Y' AND ALL_BATCH_YN = 'N' AND DATA_NM = 'LSMD_CONT_LDREG'"
        else:
